  ...
  return [
    ...
    },
    {
      ...
      handler: async (req: NextRequest, res: NextResponse) => {
        ...
        // ==================== COARSE ROLE-BASED ROUTE BLOCKING ====================
        // Note: Fine-grained permissions are enforced at component and API level
        const userRole = getRoleFromClaims(data.claims);

        // Admin-only routes
        const adminOnlyPaths = ['/home/technicians'];
        if (adminOnlyPaths.some((path) => next.startsWith(path))) {
          if (userRole !== 'admin') {
            return NextResponse.redirect(
              new URL('/home?error=unauthorized', origin).href,
            );
          }
        }

        // Supervisor+ routes (import/export)
        const supervisorPaths = ['/home/import-export'];
        if (supervisorPaths.some((path) => next.startsWith(path))) {
